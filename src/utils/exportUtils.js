...
function formatDateForCSV(order) {
  ...
  const iso = order.orderDateRaw ? order.orderDateRaw.split('T')[0] : (order.orderDate ?? '')
  return `="${iso}"`
}
...
export function generateCSV(orders) {
  ...
  orders.forEach((order, oi) => {
    ...
    const orderBase = [order.id, formatDateForCSV(order), order.customer.name, order.customer.email]
